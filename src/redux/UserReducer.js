...
export const UserReducer = createSlice({
    ...
    reducers: {
        ...
        updateUserData: (state, action) => {
            state[state.findIndex((item) => action.payload.id === item.id)] =
                action.payload;
        },
        addUser: (state, action) => {
            const newUser = action.payload;
            console.log("acp", newUser);

            state = state.push(newUser);
        },
    },
});

export const { setUserData, updateUserData, addUser } = UserReducer.actions;
